...
- 群組頁面可直接加入或移出角色卡，並顯示目前群組回覆策略
- 群組聊天室沿用 SillyTavern 原生群組生成流程：角色會讀取各自角色卡、世界書、後台生成設定與聊天上下文後主動回覆；單輪中同一角色最多回覆 2 次
...
- 群組成員與回覆策略直接寫入 SillyTavern 原生群組資料；連續群組回覆模式保持關閉，只有玩家明確送出訊息後才啟動該輪角色回覆。
